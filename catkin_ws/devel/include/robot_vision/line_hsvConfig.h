...
namespace robot_vision
{
  ...
  public:
    ...
    public:
      ...
      virtual void clamp(line_hsvConfig &config, const line_hsvConfig &max, const line_hsvConfig &min) const override
      {
      ...
      virtual void calcLevel(uint32_t &comb_level, const line_hsvConfig &config1, const line_hsvConfig &config2) const override
      {
        if (config1.*field != config2.*field)
          comb_level |= level;
      }

      virtual void fromServer(const ros::NodeHandle &nh, line_hsvConfig &config) const override
      {
        nh.getParam(name, config.*field);
      }

      virtual void toServer(const ros::NodeHandle &nh, const line_hsvConfig &config) const override
      {
        nh.setParam(name, config.*field);
      }

      virtual bool fromMessage(const dynamic_reconfigure::Config &msg, line_hsvConfig &config) const override
      {
        return dynamic_reconfigure::ConfigTools::getParameter(msg, name, config.*field);
      }

      virtual void toMessage(dynamic_reconfigure::Config &msg, const line_hsvConfig &config) const override
      {
        dynamic_reconfigure::ConfigTools::appendParameter(msg, name, config.*field);
      }

      virtual void getValue(const line_hsvConfig &config, boost::any &val) const override
      {
        val = config.*field;
      }
    };
    ...
    public:
      ...
      virtual void updateParams(boost::any &cfg, line_hsvConfig &top) const override
      {
        PT* config = boost::any_cast<PT*>(cfg);
